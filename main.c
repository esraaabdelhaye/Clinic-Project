...
void login() {
    ...
    printf("\n\t\t\t\t\t\t========== Login ==========\n");
    do {
        printf("\nYour choice (1 for username, 0 for email): ");
        scanf("%d", &choice);
        clearInputBuffer(); // Clear input buffer
        ...
        switch (choice) {
            ...
            strncpy(identifier, username, sizeof(identifier));
            break;
            case 2:
                printf("Enter your email: ");
            fgets(email, sizeof(email), stdin);
            strncpy(identifier, email, sizeof(identifier));
            default:
        ...
        // Authenticate user
        int authResult = authenticateUser(identifier, password)
...
int authenticateUser(char identifier[i], char password[]) {
    struct Patient currentpatient;

    FILE *fp = fopen("Patients.data", "r");
    if (fp != NULL) {
        while (fread(&currentpatient, sizeof(struct Patient), 1, fp)) {
            if (sizeof(identifier) == sizeof(email)) {
                if (strcmp(currentpatient.email, identifier) == 0 && strcmp(currentpatient.password, password) == 0) {
                    fclose(fp);
                    return 1; // Authentication successful
                }
            }
            else {
                if (strcmp(currentpatient.username, identifier) == 0 && strcmp(currentpatient.password, password) == 0) {
                    fclose(fp);
                    return 1; // Authentication successful
                }
            }
